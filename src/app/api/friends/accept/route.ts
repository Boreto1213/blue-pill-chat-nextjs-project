...
export async function POST(req: Request) {
    try {
        ...
        const isAlreadyFriends = await fetchRedis('sismember', `user:${session.user.id}:friends`, idToAccept);
        ...
        return new Response("Friend request accepted successfully", { status: 200 });
    } catch (error) {
